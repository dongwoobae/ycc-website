...
| AI 요약         | **Gemini 3.5 Flash** — 자막 원고 텍스트 + 세그먼트 타임스탬프 입력, 구조화 출력. 챕터 `summary`는 자기 구간(자신의 `startSeconds`부터 다음 챕터 직전까지)에서 설교자가 한 말만 담는다 — 다른 구간 내용이 섞이면 챕터를 눌러 이동한 위치에 그 내용이 없다 |
