...
| 자막 수집       | **RapidAPI YouTube Transcript API** — 배열의 `text`를 공백 결합해 원고 텍스트화. 요청에 `lang=ko`를 반드시 붙인다 — 빼면 한국어 자동자막만 있는 영상에도 영어 자동번역본이 온다(2026-09-11 확인). `text`에 남은 HTML 엔티티는 디코드한다                                                                                                                                                                                                                                                                                                                                                                                                                               |
| AI 요약         | **GPT-5.6 Sol** 1차, 실패(키 없음·HTTP 오류·시간 초과·검증 실패) 시 **Gemini 3.5 Flash → 2.5 Flash** — 자막 원고 텍스트 + 세그먼트 타임스탬프 입력, 구조화 출력. Sol을 1차로 둔 근거(2026-09-11, 설교 5편 비교): 자동자막 오기(예: 레위기→"내기", 밑그림→"미그림")를 Flash는 확인한 7건 중 5건 요약에 옮겼고, 프롬프트에 오기 교정 지시를 넣은 뒤에도 10건 중 3건을 옮겼다. Sol은 교정 지시 없이도 확인한 7건 중 0건. 문체는 합니다체로 지정한다 — 지정하지 않으면 Sol이 해라체로 쓰기도 한다. 챕터 `summary`는 자기 구간(자신의 `startSeconds`부터 다음 챕터 직전까지)에서 설교자가 한 말만 담는다 — 다른 구간 내용이 섞이면 챕터를 눌러 이동한 위치에 그 내용이 없다 |
...
④ QStash 워커  /api/jobs/summarize               (claim 기반, 서명검증)
   ...
   b. GPT-5.6 Sol, 실패 시 Gemini 3.5 Flash → 2.5 Flash(자막 원고 + 타임스탬프) → 구조화 출력
...
OPENAI_API_KEY=               # 요약 1차(GPT-5.6 Sol). 없으면 Gemini로 바로 요약
