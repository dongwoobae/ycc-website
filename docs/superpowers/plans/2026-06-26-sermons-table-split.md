...
**테스트 현실:** 모든 기존 테스트는 `vi.mock('@/lib/db')`로 DB를 목킹한다(실DB 통합 테스트 하니스 없음). 따라서 TDD는 순수 표면(스키마 컬럼 존재, `toSermon` 매핑, 순수 빌더)에만 적용한다. 쿼리/raw SQL 변경은 `pnpm build`(타입체크) + `pnpm lint` + 사용자의 수동 마이그레이션 적용으로 검증한다. 새 DB 하니스는 도입하지 않는다(YAGNI).
...
- [ ] **Step 3: `claimSermonById`를 행 보장 + CTE로**
...
- [ ] **Step 4: `forceClaimSermonById`도 동일 패턴으로**
...
- [ ] **Step 5: `fetchAndStoreTranscript`를 upsert로**
...
- [ ] **Step 6: `summarizeClaimed` set 대상을 sermon_summaries로**
...
- [ ] **Step 7: `manualSummarize`의 transcript 조회를 위성에서**
...
```bash
git add src/lib/sermons/summarize.ts
git commit -m "refactor: 요약 파이프라인(claim/summarize/retry/transcript)을 위성 테이블 기준으로 전환"
```
...
- **테스트 한계 명시**: DB 목킹 환경이라 쿼리/raw SQL은 build+lint+수동 마이그레이션 검증. 새 하니스 미도입(YAGNI). 순수 표면만 TDD(Task1, 기존 toSermon/planSermonInserts 회귀).
