...
- 📊 **자체 방문 분석** — 쿠키리스 수집(`/api/track`), 봇/관리자 제외, IP 마스킹·솔트 해시, 레이트리밋, 일일 통계 롤업 cron
...
| Test | Vitest (+ PGlite 기반 DB 통합 테스트) |
...
src/
  ...
  lib/
    ...
    analytics/                       # 봇 판별, IP 마스킹/해시, 경로 필터, 통계 롤업
    seo/                             # JSON-LD 빌더
...
| 영역 | 테스트 | 검증 대상 |
|---|---|---|
| 업로드/스토리지 | `upload-sniff`, `r2` | 허용 MIME/파일 시그니처, R2 파일명 정규화·key prefix 제한 |
| 인증/SEO | `auth-origin`, `sitemap`, `seo/jsonld` | Trusted origin 정규화, sitemap URL 생성, JSON-LD 빌더 |
| 주보 | `bulletin-editor`, `hwp/parse` | 주보 편집 데이터 변환/보정, HWP 바이너리 파싱 |
...
| 방문 분석 | `analytics/bots`, `analytics/ip`, `analytics/paths`, `analytics/server` | 봇 판별, IP 마스킹·해시, 수집 경로 필터, 체류시간 집계 |
