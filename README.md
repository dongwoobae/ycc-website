  ...
  app/
    ...
    api/
      ...
      jobs/
        ...
        retry-summaries/route.ts     # 실패 요약 재시도 + 오디오 변환 잔류 회수 (QStash cron)
...
| `retry-summaries`   | 매시간  | 실패한 요약 재시도, 오디오 변환 잔류 회수      |
...
- [ ] 관리자 작업 로그 필터링 강화
- [ ] 홈 히어로 이미지 파생본 재생성 — `church-spire.webp` 217KB 중 189KB가 화면에 표시되지 않는 픽셀이고,
      `/brand/pck-emblem.png` 25KB는 전부 낭비입니다(둘 다 `public/`이라 `next/image`가 리사이즈하지 않습니다)
- [ ] 폰트 굵기 5종 → 3종 축소 검토 — 조각 34→20개, 페이지당 400→240KB. `/news`·`/sermons` 성능 점수가
      경고선(0.8) 아래인 남은 이유지만, 디자인 변경을 수반해 보류 중입니다
